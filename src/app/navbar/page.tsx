  ...
  export default function Navbar() {
    ...
    return (
      <div className=" mx-auto flex w-full  justify-between px-4 py-1 text-sm  shadow-md">
        <section className="flex items-center gap-10">
          <Link href="/test">
            <Image src={Mylogo} alt="logo" width={50} height={15} className="hover:scale-110 transition duration-300" />
          </Link>
        ...
        {isSideMenuOpen && (
          <div className="fixed inset-0 bg-black/60 z-50 md:hidden">
            <div className="relative w-2/3 h-full font-bold text-white backdrop-blur-2xl p-5">
              <AiOutlineClose onClick={toggleSideMenu} className="cursor-pointer text-3xl" />
              <div className="mt-10 space-y-4 ">
                {navItems.map((item, index) => (
                  <div key={index} className="block p-2 text-white">
                    <Link href={item.link ?? "#"} onClick={() => toggleSubMenu(item.label)} className="flex justify-between items-center">
                      <span>{item.label}</span>
                      {item.children && (
                    ...
                    {item.children && item.label === openSubMenu && (
                      <div className="ml-4 mt-2 space-y-2">
                        {item.children.map((child, childIndex) => (
                          <Link key={childIndex} href={child.link ?? "#"} className="block px-2 py-1 text-sm text-white">
                            {child.label}
                          </Link>
                        ))}
                      </div>
